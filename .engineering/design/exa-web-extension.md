...
A manual local-path installation with no `EXA_API_KEY` configured verified that an actual Pi agent can execute both `web_search` and `web_fetch` against Hosted Exa through the `anonymous` route. Publication and registry-package verification are tracked separately in the [Initial Release Plan](../plans/active/initial-release.md).
