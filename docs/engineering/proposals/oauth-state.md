...
**Status:** Accepted for `0.2.0`; PR1–PR4 merged with CI verified; PR5 interactive login implemented locally, PR CI and Hosted smoke pending **Target:** `@hudrazine/pi-exa-web@0.2.0`

This document defines OAuth lifecycle, storage, and cross-process transaction behavior accepted on 2026-09-09. The accepted [routing design](oauth-routing.md) owns route selection, command behavior, error classification, and connection lifecycle. The [implementation plan](../plans/oauth-routing.md) owns delivery; the [verification specification](../plans/oauth-verification.md) owns acceptance checks. PR2 implements the [SQLite coordination foundation](../decisions/sqlite-state-locking.md); merged PR4 integrates non-interactive refresh, logout and revision-aware connections. Local PR5 implements explicit login and management. Required PR CI and Hosted evidence are tracked separately from local implementation.
