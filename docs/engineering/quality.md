...
| Explicit login | Real SDK discovery/register/PKCE/state/exchange, safe callbacks, temporary read-only validation, old-state preservation, cancellation/deadline, commit and connection retirement | [`tests/oauth-login.test.ts`](../../tests/oauth-login.test.ts) |
