...
The checkout implements saved routing strategies, bounded anonymous probes/cooldown and credit fallback for the next release. PR4 adds non-interactive OAuth selection and refresh; interactive management remains [planned work](../proposals/oauth-routing.md). The key is read once at extension initialization, trimmed, and never persisted.
