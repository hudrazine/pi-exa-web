...
Use `getAgentDir()` from Pi and a private `exa-web` child directory, independent of the project working directory. Respect Pi's agent-directory override through that helper; different agent directories are isolated. State is not stored in the repository, session log or model-provider auth file. There is no separate caller-configurable state path or credential import from other packages.
...
Normalize the directory identity through the real existing parent/directory so equivalent local paths coordinate. OAuth writes use the [SQLite lock contract](oauth-state-locking.md) for cancellable, bounded acquisition, transaction-long exclusion and recovery after process death. A timeout fails the OAuth operation; it never permits an unlocked write or takeover from a live owner.
...
An update with an expected revision fails with `storage-conflict` if the latest revision differs; it preserves the newer state. Explicit login uses this check at commit after its unlocked browser/validation phase. Logout and refresh serialize under the same lock. No process-local settings or credentials cache may override committed files; each tool call keeps the [routing contract's settings snapshot](anonymous-first.md).
