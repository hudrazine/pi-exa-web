...
The requirement is to keep API keys, request headers, and full request objects out of error messages. The current client nevertheless propagates upstream MCP error text and SDK/network exceptions; the renderer only sanitizes terminal display. This does not establish that arbitrary upstream error text is free of secrets. The [OAuth and routing proposal](../plans/oauth-routing-design.md#failure-classification-and-safe-output) specifies package-owned error templates and allowlisted metadata, but that protection is not implemented. Preserve the requirement when changing error handling rather than treating raw propagation as the intended safety contract.
