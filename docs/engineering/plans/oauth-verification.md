...
PR5 local evidence (2026-09-10): Linux x64 passed all 355 tests on Node 24.19.0 and 24.15.0, with actual test-process Node/OS/CPU assertions, the real Pi loader and child processes. Formatting, lint and types passed. Development used `vp run check` / `vp run test`; minimum validation used `vp env use 24.15.0` with built-in `vp check` / `vp test`, then removed the override. Pack dry run listed only LICENSE, package metadata, README and required TypeScript source, including the private login and command modules. [All four CI jobs passed on the final revision](https://github.com/hudrazine/pi-exa-web/actions/runs/34440460305) `db30761`, including macOS arm64 and Windows x64. [PR #20](https://github.com/hudrazine/pi-exa-web/pull/20) merged as `cc84794`; T7/T10 are Done. Hosted Exa and a real browser were not used; deployment compatibility remains unverified.

## PR6 Verification

T12 reconciles the README and current engineering contracts with merged PR1–PR5 behavior, preserving the historical SQLite decision. The feature mappings above and L1–L5 mappings below cover the existing suite, including real Pi loading, persisted-session secrecy, SSE initialization/cancellation and cross-process token/login conflicts. This audit required no source changes or duplicate tests. Anonymous post-publication smoke now uses the [isolated installation and execution procedure](../releases.md#verification); that procedure was documented, not executed.

Local evidence (2026-09-10, based on `cc84794`): `vp run check` passed formatting, lint and types. `vp run test` passed all 355 tests in 14 files on Linux x64 / Node 24.19.0, with explicit test-process runtime assertions, real Pi loader and child-process cases. `vp pm pack -- --dry-run --json` listed only LICENSE, README, package.json and the 11 required `src/*.ts` files, with no fixtures, saved state or build output. The existing minor Changeset covers storage, Node requirements, strategy/routing and OAuth management; the existing PR1 patch Changeset covers safe errors and connections. Both remain unchanged. `vp run changeset status` confirmed the combined minor bump toward `0.2.0`; package version remains `0.1.0`, and no version or changelog generation was run.

Operator evidence (2026-09-10): the user confirmed normal operation in Pi. Individual commands, authentication routes and restart/refresh scenarios were not specified, so this report does not establish completion of the full T13 Hosted smoke.

PR6's final-commit CI on Linux development/minimum, macOS arm64 and Windows x64 remains pending until PR creation. PR5's results do not substitute for it, and the minimum Node runtime was not rerun locally for this documentation-only change. T12 stays In review until PR6 merges with all four jobs passing. Evidence for the complete Hosted login/restart/refresh smoke (T13) remains pending. Release PR #18 merge and publication/post-publication verification (T14) were not executed by PR6.

## Lock and State Cases

PR2 coverage: `tests/state-store.test.ts` exercises L1–L2 with real child processes and SQLite, the JSON portion of L5, and settings replacement. `tests/oauth-lock.test.ts` covers L3 with controlled clocks and dependency failures. `tests/state-loader.test.ts` verifies SQLite through a test extension loaded by Pi's actual loader, the production entry and strategy command, safe storage errors through rendering/SessionManager, and CI runtime identity. PR4 covers proactive and 401-triggered L4 in `tests/oauth-process.test.ts`, and remote rotation/write/rename failures in `tests/oauth-state.test.ts` and `tests/oauth-routing.test.ts`. PR5 covers login-related conflicts and save failures in `tests/oauth-login.test.ts` and `tests/login-process.test.ts`. PR2–PR5 passed their four-job CI before merge. L4/L5 use local rotating-token services; they do not replace the Hosted restart/refresh check.
...
This is T13, separate from T12's local/CI/package checks. Run once after implementation in an isolated local Pi agent directory selected by `PI_CODING_AGENT_DIR`, with `EXA_API_KEY` unset. Install and run the intended package in that same environment, using the actual `/exa` command and Pi tools. Login requires explicit operator action; scripted external checks also require `PI_EXA_WEB_LIVE_TEST=1`. Use a different empty agent directory for T14's registry-installed anonymous smoke.
